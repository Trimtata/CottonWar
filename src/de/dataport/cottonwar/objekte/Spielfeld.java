...
public class Spielfeld extends JPanel {
	...
	public static Image background, backgroundh, backgroundw, Basis, Basis0, Krieger, Krieger0, Ritter, Ritter0, Reiter, Reiter0, Lord,
			Lord0, Drache, Drache0, Himmelswacht, Himmelswacht0;
		...
		for (Einheit e : einheiten) {
			if (e.name.contains("Basis")) {
				...
				if (e.id == 0) {
					...
					g.fillRect(e.x + 30, e.y - 20, 48, 5);
					g.setColor(Color.RED);
					g.fillRect(e.x + 30, e.y - 20, 50 * 100 / e.lpMax * e.lp / 100, 5);

				} else {
					g.setColor(Color.BLACK);
					g.fillRect(e.x + 40, e.y - 20, 48, 5);
					g.setColor(Color.RED);
					g.fillRect(e.x + 40, e.y - 20, 50 * 100 / e.lpMax * e.lp / 100, 5);
				}
			}
